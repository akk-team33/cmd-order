...
public class Main01 implements Runnable {
    ...
    private final List<Solver.Element> targetSub;
    private final List<Path> origins;
    private final Movement movement;
    private final Output out;

    public Main01(final Output out, final List<String> args) throws InitialisationException {
        try {
            this.targetRoot = normal(args.get(0));
            this.targetSub = Stream.of(args.get(1).split(Pattern.quote("/")))
                    .filter(e -> !"".equals(e))
                    .map(Solver.Element::valueOf)
                    .collect(Collectors.toList());
            this.origins = args.stream().skip(2).map(Main01::normal).collect(Collectors.toList());
        ...
        try {
            ...
            out.println("Required Arguments:")
                    ...
                    .println(
                            ...
                            Stream.of(Solver.Element.values())
                                    .map(Solver.Element::name)
